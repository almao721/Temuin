...
const laporanModel = {
  ...
  getAll: async () => {
    const [rows] = await db.execute(`
      ...
      LEFT JOIN laporan_pemenuan p ON l.id = p.laporan_id
      LEFT JOIN laporan_kelilingan kh ON l.id = kh.laporan_id
      ORDER BY l.id DESC
    `);
    return rows;
  },
  ...
  getByUserId: async (user_id) => {
    const [rows] = await db.execute(`
      SELECT l.id, l.tipe_laporan, lok.nama_lokasi, k.nama_kategori,
             p.nama_barang, kh.barang_tipe, l.created_at
      FROM laporan l
      JOIN lokasi lok ON l.lokasi_id = lok.id
      JOIN kategori k ON l.kategori_id = k.id
      LEFT JOIN laporan_pemenuan p ON l.id = p.laporan_id
  ...
  delete: async (id) => {
    await db.execute('DELETE FROM laporan_pemenuan WHERE laporan_id = ?', [id]);
    await db.execute('DELETE FROM laporan_kelilingan WHERE laporan_id = ?', [id]);
