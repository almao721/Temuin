...
const laporanModel = {
  ...
  getByUserId: async (user_id) => {
    const [rows] = await db.execute(`
      SELECT l.id, l.tipe_laporan, lok.nama_lokasi, k.nama_kategori,
             p.nama_barang, kh.barang_tipe
      FROM laporan l
      JOIN lokasi lok ON l.lokasi_id = lok.id
      JOIN kategori k ON l.kategori_id = k.id
      LEFT JOIN laporan_penemuan p ON l.id = p.laporan_id
  ...
  getByType: async (tipe_laporan) => {
    const [rows] = await db.execute(`
      SELECT l.id, l.tipe_laporan, lok.nama_lokasi, k.nama_kategori, u.nis_nip,
...
module.exports = laporanModel;const db = require('../src/config/db');
...
const laporanModel = {
  ...
  getAll: async () => {
    const [rows] = await db.execute(`
      SELECT 
        l.id, l.tipe_laporan, l.lokasi_id, l.user_id, l.kategori_id,
        lok.nama_lokasi, k.nama_kategori, u.nis_nip, u.role as user_role,
        COALESCE(p.nama_barang, '') as nama_barang,
        COALESCE(p.deskripsi, '') as penemuan_deskripsi,
        COALESCE(p.foto_barang, '') as foto_barang,
        COALESCE(kh.barang_tipe, '') as barang_tipe,
        COALESCE(kh.keterangan_lainnya, '') as keterangan_lainnya,
        COALESCE(p.waktu_insiden, kh.waktu_insiden) as waktu_insiden
      FROM laporan l
      JOIN lokasi lok ON l.lokasi_id = lok.id
      JOIN kategori k ON l.kategori_id = k.id
  ...
  // Hapus laporan double (berdasarkan data yang sama)
  deleteDuplicate: async () => {
    // Hapus laporan double berdasarkan kombinasi data yang sama
    const [result] = await db.execute(`
      DELETE l1 FROM laporan l1
      INNER JOIN laporan l2 
      WHERE l1.id > l2.id 
      AND l1.tipe_laporan = l2.tipe_laporan 
      AND l1.lokasi_id = l2.lokasi_id 
      AND l1.user_id = l2.user_id 
      AND l1.kategori_id = l2.kategori_id
    `);
    return result;
  },
