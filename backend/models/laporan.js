...
const laporanModel = {
  ...
  getAll: async ({ tipe, status, kategori_id, tanggal_mulai, tanggal_akhir, search, page = 1, limit = 10 } = {}) => {
    ...
    if (tipe && ['kehilangan', 'penemuan'].includes(tipe)) {
      where += ' AND l.tipe_laporan = ?'; params.push(tipe);
    }
    if (status) {
      where += ' AND l.status = ?'; params.push(status);
    }
    if (kategori_id) {
      where += ' AND l.kategori_id = ?'; params.push(kategori_id);
    }
    if (tanggal_mulai) {
      where += ' AND DATE(l.created_at) >= ?'; params.push(tanggal_mulai);
    }
    if (tanggal_akhir) {
      where += ' AND DATE(l.created_at) <= ?'; params.push(tanggal_akhir);
    }
    if (search) {
